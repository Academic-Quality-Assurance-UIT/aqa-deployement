...
import { useStaffSurveyCommentStatisticsQuery } from "@/gql/graphql";
import { useState, useMemo } from "react";
import { Accordion, AccordionItem, Tabs, Tab, Chip, Select, SelectItem } from "@heroui/react";
...
const TOPIC_LABELS: Record<string, string> = {
	...
	"môi trường/điều kiện làm việc": "Môi trường, Điều kiện làm việc",
	"CTĐT": "Chương trình đào tạo",
	"đào tạo, bồi dưỡng": "Đào tạo, Bồi dưỡng",
	"không xác định/không liên quan": "Chưa cụ thể/Khác",
    "ĐƠN VỊ": "Đơn vị",
...
const TOPIC_COLORS: Record<string, Color> = {
	"Hạ tầng CNTT": "emerald",
	"Cơ sở vật chất": "rose",
	"Môi trường, Điều kiện làm việc": "purple",
	"Chương trình đào tạo": "indigo",
	"Đào tạo, Bồi dưỡng": "amber",
    "Chưa cụ thể/Khác": "gray",
    "Đơn vị": "rose",
    "Chưa phân loại": "blue"
};

export default function StaffSurveyCommentRatioCharts({
	semester,
	type = ["all"],
	topic = ["all"],
}: IProps) {
	...
	const { data, loading } = useStaffSurveyCommentStatisticsQuery({
		variables: {
			semester: semester,
			type: type,
			topic: topic,
	...
	const processedData = useMemo(() => {
		if (!data?.staffSurveyCommentStatistics) return [];

		return data.staffSurveyCommentStatistics.map((item) => {
	...
	const sentimentCategories = useMemo(() => {
		...
		data?.staffSurveyCommentStatistics.forEach((item) => {
	...
	const chartTitles: Record<string, string> = {
		total: "Tổng số ý kiến theo đợt khảo sát",
				...
				<AccordionItem
					...
					title={
						<div className="flex items-center gap-2">
							<span className="font-bold text-xl font-display text-primary-dark">Thống kê ý kiến nhân viên</span>
							<Chip size="sm" color="primary" variant="flat" className="font-bold uppercase tracking-wider text-[10px]">Mới</Chip>
						</div>
					}
					subtitle="Xem thống kê tổng quan, tỉ lệ cảm xúc và khía cạnh khảo sát theo thời gian"
				>
